...
def list_elem_to_max(elem_list):
    ...
    for idx, elem in enumerate(elem_list):
        if elem != '' and elem.text is not None:
            elem_list[idx] = float(elem.text)
        else:
            elem_list[idx] = np.nan
...
def get_uni_attrib(str_elem, bldg_elem_list, gml_root, dataset_name):
    ...
    if dataset_name == 'vantaa-gov':
        ...
        for elem in bldg_elem_list:
            try:
                if elem.find(".//{}".format(str_elem), gml_root.nsmap) is not None:
        ...
        return([elem.find(".//{}".format(str_elem), gml_root.nsmap).text if elem.find(".//{}".format(str_elem), gml_root.nsmap) is not None else None for elem in bldg_elem_list])
...
def get_curr_use_attrib(var_elem, bldg_elem_list, gml_root, key):
    ...
    if None not in list_curr_use_full:
        # if no None directly return list elems
        return [elem.attrib[key] for elem in list_curr_use_full if elem is not None]
    else:
        # get attrib for all elements that are not None
        list_curr_use = [elem.attrib[key] for elem in list_curr_use_full if elem is not None]
        # get indexes of list entries that have a current use value
        index_not_none = np.where(np.array(list_curr_use_full, dtype=object) is not None)[0].tolist()
...
def get_min_heights_roof(
        bldg_elem_list,
        gml_root,
        roof_elem='bldg:RoofSurface//gml:posList',
...
def get_roof_type_from_lod2(ft_elem, bldg_elem_list, gml_root):
    ...
    for idx, roof in enumerate(list_roof_elems):
        ...
        if len(roof) == 1:
            roof_type[idx] = is_roof_flat_or_slanted(roof[0])
            ...
            sys.exit('Looks like we did not fetch roof elements correctly previously.')
