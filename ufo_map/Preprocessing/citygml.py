...
def citygml_to_df(cityGML_buildings,
    ...
    * file info: country, region, city, district, file_name. Pass the info as a vector.
    ...
    for i, building in enumerate(tqdm(cityGML_buildings)):
        ...
        # compute height
        height_array[i] = min_max_wall_height_calculator(
            building, './/bldg:WallSurface//gml:posList', cityGML_root.nsmap)
    ...
    if bbox:
        df['height_bbox'] = height_bbox_array
    if measured_height:
        df['measured_height'] = measured_height_array
