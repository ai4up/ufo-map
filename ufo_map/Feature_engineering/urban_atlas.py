...
def point_in_ua(geometries, ua_sindex, geometries_ua, classes, buffer_size):
    ...
    for index, point_geometry in enumerate(geometries):
        ...
        if (len(ua_indexes_containing_the_point) > 1):
            print(
                f" the same point have >1 class of UA {point_geometry}! Classes indexes: {ua_indexes_containing_the_point}")
            inter_areas = [geometries_ua[i].intersection(point_geometry.buffer(
                buffer_size)).area for i in ua_indexes_containing_the_point]
        ...
        if (ua_indexes_containing_the_point_index == []):
            ...
            continue
        points_classes[index] = [classes[i]
                                 for i in ua_indexes_containing_the_point][ua_indexes_containing_the_point_index]
